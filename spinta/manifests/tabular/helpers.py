...
ALLOWED_NESTING_TYPES = ALLOWED_PARTIAL_TYPES + ALLOWED_ARRAY_TYPES + [DataTypeEnum.TEXT.value]
...
def _detect_header(
    ...
    if unknown_columns:
        ...
        raise TabularManifestError(f"{path}:{line}: Unknown columns: {unknown_columns}.")
    return header
...
def _detect_dimension(
    ...
    if len(dimensions) > 1:
        ...
        raise TabularManifestError(
            f"{path}:{line}: In one row only single dimension can be used, but found more than one: {dimensions}"
        )

    if row["type"]:
        if row["type"] not in EXTRA_DIMENSIONS:
            raise TabularManifestError(f"{path}:{line}:type: Unknown additional dimension name {row['type']}.")
        return row["type"]
...
class TabularReader:
    ...
    def __init__(self, state: State, path: str, line: str, allow_updates: bool = False):
        self.state = state
        self.path = path
        self.line = line
        self.data = {}
    ...
    def append(self, row: Dict[str, str]) -> None:
        if not self.allow_updates:
            if any(row.values()):
                self.error(f"Updates are not supported in context of {self.type!r}.")
...
class DatasetReader(TabularReader):
    ...
    def release(self, reader: TabularReader = None) -> bool:
        return (
            ...
            or (isinstance(reader, (ResourceReader, ModelReader)) and self.name == "/")
        )
...
class ResourceReader(TabularReader):
    ...
    def read_backend(self, row: Dict[str, str]) -> None:
        ...
        if self.name in backends:
            self.error(f"Backend {self.name!r} with the same name already defined.")
    ...
    def release(self, reader: TabularReader = None) -> bool:
        ...
        return (
            reader is None
            or isinstance(reader, (ManifestReader, DatasetReader, ResourceReader, EnumReader, PrefixReader))
    ...
    def read(self, row: Dict[str, str]) -> None:
        ...
        if self.name == "/":
            ...
            self.data = {
                ...
                "pk": ([x.strip() for x in row["ref"].split(",")] if row["ref"] else []),
                "level": row["level"],
            }
...
class ModelReader(TabularReader):
    ...
    def release(self, reader: TabularReader = None) -> bool:
        return reader is None or isinstance(
            reader,
            (
                ManifestReader,
                DatasetReader,
...
def _get_type_repr(dtype: DataType | str | None) -> str:
    ...
    if isinstance(dtype, DataType):
        ...
        if dtype.type_args:
            ...
            args = f"({args})"
        dtype_name = (
            dtype.name if not isinstance(dtype, (Denorm, Inherit, ExternalRef, ArrayBackRef)) else dtype.get_type_repr()
        )
        ...
        if additional_args:
            if [
                additional_arg
                for additional_arg in additional_args.split(" ")
                if additional_arg not in [required.strip(" "), unique.strip(" ")]
            ]:
                raise TabularManifestError
...
class PropertyReader(TabularReader):
    ...
    def release(self, reader: TabularReader = None) -> bool:
        return reader is None or isinstance(
            reader,
            (ManifestReader, DatasetReader, ResourceReader, BaseReader, ModelReader, PropertyReader, UniqueReader),
        )
    ...
    def _append_prepare(self, row: Dict[str, str], prepare: str):
        ...
        self.data["prepare_given"].append(PrepareGiven(appended=True, source=row["source"], prepare=row["prepare"]))
...
def _datatype_handler(reader: PropertyReader, row: dict, initial_data_loader: Callable[[str, dict, dict], dict]):
    ...
    if row["prepare"]:
        new_data["prepare_given"].append(PrepareGiven(appended=False, source="", prepare=row["prepare"]))
...
def _text_datatype_handler(reader: PropertyReader, row: dict):
    ...
    c_lang_data = _empty_property(
        _initial_normal_property_schema(
            c_lang_name,
            dtype,
            {
...
def _process_allowed_partial_type(
    reader: PropertyReader, current_parent: dict, root_name: str, empty_array_row: dict
) -> dict:
...
def _get_parent_data_text(reader: PropertyReader, given_row: dict, full_name: str, current_parent: dict):
    ...
    if current_parent["type"] in ALLOWED_ARRAY_TYPES:
        if current_parent["items"] and current_parent["items"]["type"] != DataTypeEnum.TEXT.value:
            raise NestedDataTypeMismatch(
                initial=current_parent["properties"][name]["type"], required=DataTypeEnum.TEXT.value
            )
        elif not current_parent["items"]:
        ...
        if (
            name in current_parent["properties"]
            and current_parent["properties"][name]["type"] != DataTypeEnum.TEXT.value
        ):
            raise NestedDataTypeMismatch(
                initial=current_parent["properties"][name]["type"], required=DataTypeEnum.TEXT.value
            )
        elif name not in current_parent["properties"]:
            current_parent["properties"][name] = _empty_property(_text_datatype_handler(reader, empty_text_row))
        current_parent = current_parent["properties"][name]
    return current_parent


def _extract_and_create_parent_data(reader: PropertyReader, current_row: dict, property_path: str) -> (dict, dict, str):
    ...
    for i, prop in enumerate(split_props):
        ...
        if i + 1 != count:
            if not current_parent and full_prop:
                ...
                current_parent = _get_parent_data_partial(
                    reader, given_row, ".".join(full_name), current_parent or full_prop
                )
        else:
...
def _check_if_property_already_set(reader: PropertyReader, given_row: dict, full_name: str):
    ...
    for name in split:
        ...
        if "[]" in name:
            ...
            base = _extract_children_from_nested(base, name)
    if base and base["given_name"] == full_name and base["explicitly_given"]:
        reader.error(
...
class PrefixReader(TabularReader):
    ...
    def read(self, row: Dict[str, str]) -> None:
        ...
        if self.name in prefixes:
            self.error(
                f"Prefix {self.name!r} with the same name is already defined for this {node.name!r} {node.type}."
            )
...
class NamespaceReader(TabularReader):
    ...
    def read(self, row: Dict[str, str]) -> None:
        ...
        if self.name in manifest.namespaces:
            self.error(f"Namespace {self.name!r} with the same name is already defined.")
...
class ParamReader(TabularReader):
    ...
    def _check_param_name(self, node: TabularReader, name: str) -> None:
        if "params" in node.data and name in node.data["params"]:
            self.error(f"Parameter {name!r} with the same name already defined!")
...
class EnumReader(TabularReader):
    ...
    def read(self, row: ManifestRow) -> None:
        ...
        if source in enum:
            self.error(f"Enum {self.name!r} item {source!r} with the same value is already defined.")
...
class LangReader(TabularReader):
    ...
    def read(self, row: ManifestRow) -> None:
        ...
        if not isinstance(
            reader, (DatasetReader, ResourceReader, BaseReader, ModelReader, PropertyReader, EnumReader, LangReader)
        ):
        ...
        if self.name in lang:
            self.error(
                f"Language {self.name!r} with the same name is already defined for this {reader.name!r} {reader.type}."
            )
...
class UniqueReader(TabularReader):
    ...
    def read(self, row: ManifestRow) -> None:
        ...
        if not isinstance(reader, (ModelReader, UniqueReader, AppendReader)):
...
def _check_row_size(
    ...
    if len(header) != len(row):
        header = ["header", "row"]
        table = [["∅" if x is None else x for x in v] for v in zip_longest(header, row)]
    ...
    for sheet in wb:
        ...
        for i, row in enumerate(rows, 2):
            ...
            if empty_rows(row) > 100:
                log.warning(f"Too many consequent empty rows, stop reading {path} at {i} row.")
                break
...
def referenced_model_name(
    ...
    if explicit_only and explicit:
        keys = refprops
    elif not explicit_only and ((refprops and pkeys != refprops) or explicit):
        keys = refprops
...
def _namespaces_to_tabular(
    ...
    *,
    ...
    namespaces = {k: ns for k, ns in namespaces.items() if not ns.generated}
    first = True
...
def _unique_to_tabular(model_unique_data, hide_list: List) -> Iterator[ManifestRow]:
    ...
    for row in model_unique_data:
        if row not in hide_list:
            yield torow(DATASET, {"type": "unique", "ref": ", ".join([r.name for r in row])})
    ...
    for param in params:
        for i, (source, prepare) in enumerate(zip(param.source, param.prepare)):
            ...
            if i == 0:
                ...
                yield torow(DATASET, {"source": source, "prepare": prepare})
    ...
    *,
    ...
    yield torow(
        DATASET,
        {
            ...
            "type": resource.type,
            "ref": (backend.name if (external and backend and backend.origin != BackendOrigin.resource) else ""),
    ...
    *,
    ...
    if external and prop.external:
        if isinstance(prop.external, list):
            ...
            # )
            raise DeprecationWarning("Source can't be a list, use prepare instead.")
        elif prop.external:
...
def _prepare_to_tabular(data, prop):
    ...
    if prop.given.prepare:
        ...
        for prep in prop.given.prepare:
            if prep["appended"]:
                prep_rows.append(torow(DATASET, {"source": prep["source"], "prepare": prep["prepare"]}))
            else:
                if prop.external:
    ...
    *,
    ...
    if external and model.external:
        ...
        if not model.external.unknown_primary_key and all(p.access >= access for p in model.external.pkeys):
            ...
            # resulting manifest.
            data["ref"] = ", ".join([p.name for p in model.external.pkeys])
    ...
    for model in models:
        ...
        if model.external:
            ...
            if (
                ...
                and (resource is None or resource.name != model.external.resource.name)
            ):
...
def render_tabular_manifest_rows(
    ...
    *,
    ...
    if sizes is None:
        sizes = dict(
            [(c, len(c)) for c in cols[:hs]] + [(c, 1) for c in cols[hs:he]] + [(c, len(c)) for c in cols[he:]]
        )
        rows = list(rows)
        for row in rows:
    ...
    *,
    ...
    if check_column_names and invalid:
        if len(invalid) == 1:
            ...
            raise MultipleErrors(PropertyNotFound(property=col) for col in invalid)
    return result
